...
(function() {
	var canvas, ctx, track_mouse, mouse = {}, draw, body, H, W, particlemotion, Particle, particles = [], randSpeed, particlesNum;
	...
	randSpeed = function(speed) {
		return Math.round(Math.random()) ? Math.round(Math.random() * 9 + 1)
	...
	particlemotion = function() {
		...
		for (i = 0; i < particlesNum; i++) {
			draw(particles[i].x, particles[i].y, particles[i].r,
					particles[i].rgb);
			var x = particles[i].x - mouse.x, dx = Math.abs(x), y = particles[i].y
					- mouse.y, dy = Math.abs(y), dt = Math.sqrt(dx * dx + dy
					* dy);
